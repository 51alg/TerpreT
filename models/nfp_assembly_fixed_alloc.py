...
maxScalar = 1 + inputStackSize + numTimesteps
...
# Runtime state
stackCarValue = Var(maxScalar)[numTimesteps]
stackCdrValue = Var(maxScalar)[numTimesteps]
    ...
    if isHalted[t] == 0:
        with instrPtr[t] as ip:  # !! factor: numTimesteps * numInstructions
            ...
            with arg1Val as p:  # !! factor: numTimesteps * numInstructions * maxScalar
                ...
                elif p <= inputStackSize + t:
                    tmpArg1DerefCarValue[t].set_to(stackCarValue[p - inputStackSize - 1])
                    tmpArg1DerefCdrValue[t].set_to(stackCdrValue[p - inputStackSize - 1])
                else:
                    tmpArg1DerefCarValue[t].set_to(0)
                    tmpArg1DerefCdrValue[t].set_to(0)
            ...
            if tmpIsRegInstr[t] == 0:
                ...
                if tmpRegInstr[t] == 0:  # cons
                    tmpOutVal[t].set_to(1 + inputStackSize + t)
            ...
            # Extend stack for this timestep, only Cons allocates
            tmpDoPushStack[t].set_to(IsCons(instruction))
            if tmpDoPushStack[t] == 1:
                stackCarValue[t].set_to(arg1Val)
                stackCdrValue[t].set_to(arg2Val)
            elif tmpDoPushStack[t] == 0:
                stackCarValue[t].set_to(0)
                stackCdrValue[t].set_to(0)
        ...
        if outputListIsDone[n] == 1:
            ...
            with outputListCopyPos[n] as p:
                if p == 0:
                    ...
                    outputListVal[n].set_to(stackCarValue[p - inputStackSize - 1])
                    outputListCopyPos[n + 1].set_to(stackCdrValue[p - inputStackSize - 1])
